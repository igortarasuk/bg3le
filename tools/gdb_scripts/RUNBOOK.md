...
`reference/ACHIEVEMENTS-DIAGNOSIS.md`. Paths below are relative to the
...
- Environment variables must reach the `bg3` process inside the Steam runtime
  ...
  (`BG3LE_ACHIEVEMENTS=0 %command%`) or in whatever launcher you use to
  preload `libbg3le.so`.
...
| `entry_watch_gen.py` | Older generator: non-interrupting entry breakpoint on a raw VMA, logs `rdi`/`rsi`. Use it in section (e) to watch chain frames. |

## (a) The Steam reset helper

`reset_achievements` is a small standalone C program against the game's
`libsteam_api.so` (`--list` walks `GetNumAchievements`/`GetAchievement`,
`--clear NAME` calls `ClearAchievement` then `StoreStats`). It is not part
of this repository; build it next to these scripts:
...
1. Build bg3le from the tree that carries the predicate patch
   (`ensure_achievement_gate_patch` in `src/preload.cpp`).
...
6. Control: repeat steps 2-5 once with `BG3LE_ACHIEVEMENTS=0`. The badge
...
Goal: the engine's real Osiris handler (`FUN_03867780`) must pass
`HasCustomMods` and reach `ISteamUserStats::SetAchievement` on its own.

Builds before the predicate patch shipped a DIV-dispatch bypass that called
Steam directly from `UnlockAchievement`; it was also controlled by
`BG3LE_ACHIEVEMENTS=0`. Make sure the build under test carries
`ensure_achievement_gate_patch` (the log line in (b) step 3 proves it), or a
toast may come from the bypass rather than from the engine path.
...
2. Launch with mods enabled. Load a modded save into gameplay.
3. Open the Lua console. bg3le runs a bg3lua-compatible debugger server on
   ...
   - `CreateConsole`: with `"CreateConsole": true` in
     ...
     terminal running `client/bg3lua` (a dependency-free Python 3 script)
     at startup.
   - Manual: from any terminal, `client/bg3lua --host 127.0.0.1 --port 9998`
...
5. Expected within a few seconds:
   ...
   - in the shared log, `ISteamUserStats::SetAchievement(` ... followed by
     `-> true` (with `BG3LE_DUMP_ACHIEVEMENT_STACKS=1` also a stack dump);
...
This is the reference chain for locating any gate that remains after the
predicate patch. Do it with **mods disabled** (vanilla profile, no mod
...
6. Read `/tmp/steam_bt_child.log`. Each hit gives
   ...
   `reference/` as the mods-off baseline (for example
...
## (f) A/B with BG3LE_ACHIEVEMENTS=0
...
| B | `BG3LE_ACHIEVEMENTS=0` | present | no `SetAchievement` line, `--list` not earned |
...
2. Launch with the run's env. Confirm in the log: run A shows
   ...
   `EnableAchievements: BG3LE_ACHIEVEMENTS=0, predicate left unpatched`.
...
5. Record results in `reference/ACHIEVEMENTS-DIAGNOSIS.md` with the
