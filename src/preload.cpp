...
// Off when "EnableAchievements": false in ScriptExtenderSettings.json
// (bg3se parity, default on). BG3LE_NO_ACH_PATCH=1 forces off.
bool achievement_patch_disabled() {
    static const bool disabled = [] {
        const char* e = std::getenv("BG3LE_NO_ACH_PATCH");
        if (e != nullptr && *e != '\0' && std::strcmp(e, "0") != 0) {
            logf("EnableAchievements: BG3LE_NO_ACH_PATCH set, predicate left unpatched");
            return true;
        }
        char exe[4096];
...
void* maybe_wrap_div_table(void* init_fn) {
    ...
    // through whatever handler osi::set_handlers was given -- it never goes
    // through this DIV table at all, so it needs the real pointers set here
    // regardless of whether diagnostic wrapping (below) is enabled. The
    // achievement mod-gate no longer needs interposing on this path at all --
    // see install_achievement_gate_patch.
...
extern "C" long _ZNK7COsiris13NoStoryLoadedEv(void* self) {
    ...
    if (last == 0.0) last = t;
    if (t - last >= 5.0) {
        // Rate of this getter being polled -- NOT whether a story is loaded
        // (that's StoryLoaded()/OnAfterOsirisLoad above). Named after the
        // engine symbol it hooks, which is confusingly named for a log line.
        logf("pump: NoStoryLoaded() poll rate %.1f calls/s", n / (t - last));
        calls.store(0);
        last = t;
    }
...
bool hooked_set_achievement(void* self, const char* name) {
    ...
    // signals *other* threads and waits), so it never shows who actually
    // called SetAchievement. dump_own_stack() unwinds this thread directly.
    dump_own_stack("SetAchievement call");
...
__attribute__((constructor)) static void bg3le_init() {
    log_init();
    logf("bg3le loaded");
    // Before main and the fork, so session-load caches see it.
